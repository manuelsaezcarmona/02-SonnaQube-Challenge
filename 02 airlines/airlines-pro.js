...
function airLines() {
  ...
  flightsWithScale.forEach((flight) => {
    console.log(
      `ID: ${flight.id}   DESTINO: ${flight.to}   COSTE: ${flight.cost}`
    );
  });
  ...
  for (let i = flights.length - 1; i > flights.length - 1 - 5; i--) {
    console.log(`El vuelo con origen: ${flights[i].from} y destino ${
      flights[i].to
        ...
        tiene un coste de ${flights[i].cost} y ${isScale(
      flights[i].scale
    )} ID: ${flights[i].id}
        `);
  }
...
function printFlights(flightsArr) {
  flightsArr.forEach((flight) => {
    console.log(
      `ID: ${flight.id}  ORIGEN: ${flight.from} DESTINO: ${flight.to}   COSTE: ${flight.cost}`
    );
  });
}
...
function deleteFlight(flightsArr) {
  ...
  let indexflight = flightsArr.findIndex(
    (flight) => flight.id === flightDelete
  );
...
function purchaseFlight(selectedFlights) {
  ...
  do {
    idFlight = parseInt(
      prompt("Introduce el ID del vuelo que quieres comprar")
    );
